...
define(['N/runtime', 'N/search', 'N/url', "N/redirect", "N/log", "N/task", 'N/record'],
    ...
    (runtime, search, url, redirect, log, task, record) => {
        ...
        const getMatchedPurchaseOrders = () => {
            const searchId = runtime.getCurrentScript().getParameter({name: "custscript_dd_specify_savedsearch"});
            ...
            let filterArr = [
                ...
                {name: "custbody_dd_purchasecontract", operator: search.Operator.ANYOF, values: "@NONE@"}
            ];
        ...
        const getInputData = (inputContext) => {
            const pcIdArr = getMatchedPurchaseOrders();
            log.debug("pcIdArr", pcIdArr);
            log.debug("pcIdArr.length", pcIdArr.length);
        ...
        const map = (mapContext) => {
            // Usage limit 1000
            const poId = mapContext.value && JSON.parse(mapContext.value);
            // recovery dd purchase contract field value for PO
            if (poId){
                const poRecord = record.load({type: record.Type.PURCHASE_ORDER, id: poId});
                const ddPc = poRecord.getValue({fieldId: "custbody_dd_purchasecontract"});
                if (!ddPc){
                    const pcId = poRecord.getValue({fieldId: "purchasecontract"});
                    log.debug("PO: " + poId + " PC: " + pcId);
                    if (pcId){
                        record.submitFields({
                            type: record.Type.PURCHASE_ORDER,
                            id: poId,
                            values: {
                                custbody_dd_purchasecontract: pcId
                            },
                            options: {
