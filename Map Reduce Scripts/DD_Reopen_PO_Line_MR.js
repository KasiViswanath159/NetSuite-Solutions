...
define(['N/runtime', 'N/search', 'N/url', "N/redirect", "N/log", "N/task", 'N/record'],
    ...
    (runtime, search, url, redirect, log, task, record) => {
        ...
        const getClosedPO = () => {
            const searchId = runtime.getCurrentScript().getParameter({name: "custscript_po_closed_line_search_id"});
            ...
            if (poLineSearch){
                const columnArr = [
                    ...
                    {name: "linesequencenumber", outKey: "line"},
                ]
                ...
                if (data && data.length > 0){
                    ...
                    data.forEach(poLineInfo => {
                        if (!dataInfo.hasOwnProperty(poLineInfo.id)){
                            dataInfo[poLineInfo.id] = [];
                        }
                        dataInfo[poLineInfo.id].push({
                            item: poLineInfo.item,
                            line: poLineInfo.line
                        });
                    });
                    data = dataInfo;
                }
        ...
        const getInputData = (inputContext) => {
            let pcLineIdInfo = getClosedPO();
            log.debug("pcLineIdInfo", pcLineIdInfo);
            let poLineInfoArr = [];
            if (pcLineIdInfo){
                for (const poId in pcLineIdInfo){
                    poLineInfoArr.push({poId: poId, lineArr: pcLineIdInfo[poId]});
                }
            }
            // Get search result and pass it to map entry point.
            return poLineInfoArr;
        }
        ...
        const map = (mapContext) => {
            // Usage limit 1000
            const poInfo = mapContext.value && JSON.parse(mapContext.value);
            const poId = poInfo && poInfo.poId;
            const closedItemLineArr = poInfo && poInfo.lineArr;
            // update po
            if (poId && closedItemLineArr && closedItemLineArr.length > 0){
                const poRecord = record.load({type: record.Type.PURCHASE_ORDER, id: poId});
                closedItemLineArr.forEach(lineInfo => {
                    //CLOSED
                    poRecord.setSublistValue({sublistId: "item", fieldId: "isclosed", value: false, line: lineInfo.line - 1});
                });
