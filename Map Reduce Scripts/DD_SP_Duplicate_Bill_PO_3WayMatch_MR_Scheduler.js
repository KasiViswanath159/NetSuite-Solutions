    ...
    (record, search, runtime, format) => {
        ...
        const getInputData = (inputContext) => {
            ...
            try {
                ...
                let shortPayPurchaseOrderSavedSearchId= runtime.getCurrentScript().getParameter({ name: 'custscript_duplicate_bills_sp_not_procsd' });
        ...
        const reduce = (reduceContext) => {
            ...
            try {
                ...
                if (searchResultCount > 0) {
                    ...
                    if(dateDifferenceInDays >= 2 && vendorArray.indexOf(vendorVal) == -1) IRDateFlag = true;
                    if(dateDifferenceInDays >= 3 && vendorArray.indexOf(vendorVal) != -1) IRDateFlag = true;
                }
        ...
        const processShortPay = (purchase_order_internal_id, vendor_bill_internal_id, reason_code_obj) => {
            ...
            // load the purchase order and get tran id
            var poRecObj2 = record.load({ type: 'purchaseorder', id: purchase_order_internal_id });
            var tranIdVal = poRecObj2.getValue('tranid');

            // get the credit line values
            var creditLineValues = getCreditLineValues(tranIdVal);
            log.debug('from line 516: creditLineValues', creditLineValues);
            log.debug('from line 516: creditLineValues.length', creditLineValues.length);
            ...
            var purchaseorderSearchObj2 = search.create({
                ...
                filters:
                    [
                        ...
                        "AND",
                        ["billingtransaction.internalid","anyof",relatedBillsArray2]
                    ],
                columns:
                    [
                        search.createColumn({
            ...
            var purchaseorderSearchObj3 = search.create({
                ...
                filters:
                    [
                        ...
                        ["taxline","is","F"],
                        "AND",
                        ["custcol_pwc_addedbymuleboolean","is","T"], "AND",
                        ["billingtransaction.internalid","anyof",relatedBillsArray2]
                    ],
                columns:
                    [
            ...
            purchaseOrderSearchObj.run().each(function(result){
                ...
                log.debug('itemObj from line 1025', itemObj);
                ...
                if (poUnits && vbUnits && vbUnits == poUnits) {
                    ...
                    if (vbUomItemRate > poUOMItemRate && vbQuantity > receivedQuantity) {
                        ...
                        log.debug('before calling get rate of unit: from line 1060');
                        ...
                        itemObj.itemDescription = "The rate for item " + itemNameText + " in Vendor Bill is " + parseFloat(Math.abs(quatityRateDifferenceAmount)).toFixed(2) + " higher than the rate in Purchase Order.";
                        ...
                        itemObj.extraLineDescription = "The quantity for item " + itemNameText + " in Vendor Bill is " + Math.abs(itemQuantityDifference) + " lesser than the quantity in Item Receipt.";
                        ...
                        var creditLine = creditLineValues.find(({ itemText, itemIdVal }) => itemIdVal == itemObj.itemNameText && itemText == "PRICE_HIGHER");
                        log.debug('creditLine from line 1100', creditLine);
                        let itemRateDifference = vbUomItemRate - poUOMItemRate;
                        if (creditLine) itemRateDifference = itemRateDifference + parseFloat(creditLine.amountVal);
                        ...
                        log.debug('before calling get rate of unit: from line 1113');
                        ...
                        itemObj.itemDescription = "The quantity for item " + itemNameText + " in Vendor Bill is " + Math.abs(itemQuantityDifference) + " lesser than the quantity in Item Receipt.";
                        itemObj.itemType = itemShortShipId;
                        itemObj.itemTypeText = 'ITEM_SHORTSHIP';
                        itemObj.isEaches = false;
                        itemObj.isQtyRateDifference = true;
                        itemObj.rateDifference = 0;
                        itemObj.extraLineDescription = "The rate for item " + itemNameText + " in Vendor Bill is " + parseFloat(Math.abs(quatityRateDifferenceAmount)).toFixed(2) + " higher than the rate in Purchase Order.";
                        ...
                        var creditLine = creditLineValues.find(({ itemText, itemIdVal }) => itemIdVal == itemObj.itemNameText && itemText == "ITEM SHORTSHIP");
                        log.debug('creditLine from line 1150', creditLine);
                        let itemQuantityDifference = vbQuantity - receivedQuantity;
                        if (creditLine) itemQuantityDifference = itemQuantityDifference + parseFloat(creditLine.quantityVal);
                        itemObj.quantityDifference = itemQuantityDifference;
                        itemObj.itemDescription = "The quantity for item " + itemNameText + " in Vendor Bill is " + Math.abs(itemQuantityDifference) + " lesser than the quantity in Item Receipt.";
                        itemObj.itemType = itemShortShipId;
                        itemObj.itemTypeText = 'ITEM_SHORTSHIP';
                ...
                log.debug('poLineAmountVal, from line 1194', poLineAmountVal);
                log.debug('vbLineAmountVal, from line 1195', vbLineAmountVal);
                ...
                if (poUnits && vbUnits && poUnits != vbUnits && poLineAmountVal != vbLineAmountVal) {
                    ...
                    log.debug('before calling get rate of unit: from line 1202');
                    ...
                    if (parseFloat(vbSingleQuantityRate) > parseFloat(poSingleQuantityRate)) {
                        ...
                        var creditLine = creditLineValues.find(({ itemText, itemIdVal }) => itemIdVal == itemObj.itemNameText && itemText == "PRICE_HIGHER");
                        log.debug('creditLine from line 1291', creditLine);
                        if (creditLine) totalItemRateDifference = totalItemRateDifference + parseFloat(creditLine.amountVal);
                        //
                    ...
                    if (parseFloat(vbQuantity) > parseFloat(receivedQuantity)) {
                        ...
                        var creditLine = creditLineValues.find(({ itemText, itemIdVal }) => itemIdVal == itemObj.itemNameText && itemText == "ITEM SHORTSHIP");
                        log.debug('creditLine from line 1319', creditLine);
                        if (creditLine) itemQuantityDifference = itemQuantityDifference + parseFloat(creditLine.quantityVal);
                        itemObj.quantityDifference = itemQuantityDifference;
                        itemObj.itemDescription = "The quantity for item " + itemNameText + " in Vendor Bill is " + Math.abs(itemQuantityDifference) + " lesser than the quantity in Item Receipt.";
                        itemObj.itemType = itemShortShipId;
                        ...
                        if (creditLine) itemObj.rateDifference = itemObj.rateDifference + parseFloat(creditLine.amountVal);
                        log.debug('itemObj.rateDifference from line 1345', itemObj.rateDifference);
                        if (isAllowAllItems || reason_code_obj.accuratePayReasonCodeId == itemShortShipId) varianceArray.push(itemObj);
                    }
                }
                return true;
        ...
        function createVendorCredit(variance_array, item_not_on_po_bills_array, discount_array, reasoncode_obj, is_allow_all_items, mule_items_main_array, po_internal_id, vendor_bill_internal_id, po_tran_id) {
            ...
            for (var lineNumber = 0; lineNumber < variance_array.length; lineNumber++) {
                ...
                if (variance_array[lineNumber].itemTypeText == 'ITEM_DISCOUNT') {
                    ...
                    log.debug('before calling get rate of unit: from line 1568');
        ...
        function getAllRelatedBills(po_internal_id) {
            ...
            var vendorbillSearchObj = search.create({
                ...
                filters: [
                    ...
                    [ 'createdfrom', 'anyof', po_internal_id ], 'AND',
                    [ 'mainline', 'is', 'T' ], 'AND',
                    ['approvalstatus','noneof','2'], 'AND',
                    ['custbody_db_sp_processed','is','F'], 'AND',
                    ['custbody_created_from_shortpay','is','F'],
                ],
                columns: [
        ...
        function getBills(po_internal_id) {
            ...
            var vendorbillSearchObj = search.create({
                ...
                filters: [
                    ...
                    [ 'createdfrom', 'anyof', po_internal_id ], 'AND',
                    [ 'mainline', 'is', 'T' ], 'AND',
                    [ 'status', 'noneof', 'VendBill:B', 'VendPymt:D' ], 'AND',
                    ['custbody_db_sp_processed','is','F'], 'AND',
                    ['custbody_created_from_shortpay','is','F'],
                ],
                columns: [
        ...
        function getCreditLineValues(po_tran_id) {
            ...
            var vendorcreditSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        ["memo","isnotempty",""],
                        "AND",
                        ["createdfrom.custbody_dd_purchase_order_reference","is",po_tran_id],
                        "AND",
                        ["createdfrom.custbody_created_from_shortpay","is","T"]
                    ],
                columns:
                    [
                        ...
                        search.createColumn({name: "custcol_item_description", label: "Item Description"}),
                        search.createColumn({name: "quantity", label: "Quantity"})
                    ]
            });
            ...
            vendorcreditSearchObj.run().each(function(result){
                // .run().each has a limit of 4,000 results
                var itemText = result.getText('item');
                var amountVal = result.getValue('amount');
                var memoVal = result.getValue('memo');
                var quantityVal = result.getValue('quantity');
                var itemIdVal = (memoVal.slice(memoVal.indexOf('item '), memoVal.indexOf(' in'))).split(" ")[1];
