    ...
    (log, record, runtime, search) => {
        ...
        const getRemainingDeleteData = () => {
            ...
            let configSearch = search.create({
                type: "customrecord_dd_new_items_for_vendor",
                columns: columnInfoArr ? columnInfoArr.map(columnInfo => columnInfo.colInfo) : [],
                filters: filterInfoArr
            });

            const deleteData = getAllData(configSearch, columnInfoArr);
            let deleteIdArr = [];
            deleteData && deleteData.length > 0 && deleteData.forEach(dataInfo => {
                deleteIdArr.push(dataInfo.id);
            });
            return deleteIdArr;
        }
        /**
        ...
        const getInputData = (inputContext) => {
            let remainingIdArr = [];
            try {
                remainingIdArr = getRemainingDeleteData();//[4255983];//
            } catch (e) {
                log.debug("Exception on map", e);
            }
            return remainingIdArr;
        }
        ...
        const map = (mapContext) => {
            try {
                const id = mapContext.value || "";
                id && record.delete({type: "customrecord_dd_new_items_for_vendor", id: id});
            } catch (e) {
                log.debug("Exception on map", e);
            }
        }
