...
define(['N/runtime', 'N/search', 'N/url', "N/redirect", "N/log", "N/task", 'N/record'],
    ...
    (runtime, search, url, redirect, log, task, record) => {
        ...
        const getAllPurchaseContracts = () => {
            let filterArr = [
                {name: "type", operator: search.Operator.ANYOF, values: "PurchCon"},
        ...
        const getInputData = (inputContext) => {
            const pcIdArr = getAllPurchaseContracts();
            log.debug("pcIdArr Count:" + pcIdArr.length, pcIdArr);
            // Get search result and pass it to map entry point.
            return [4248837];//pcIdArr;
        }
        ...
        const map = (mapContext) => {
            // Usage limit 1000
            const pcId = mapContext.value && JSON.parse(mapContext.value);
            try{
                // remove items for PC
                if (pcId){
                    const pcRecord = record.load({type: record.Type.PURCHASE_CONTRACT, id: pcId});
                    const sublistId = "item";
                    let hasRemoveItem = false;
                    for (let line = 0; line < pcRecord.getLineCount({sublistId: sublistId}); line++){
                        const itemName = pcRecord.getSublistText({sublistId: sublistId, fieldId: sublistId, line: line});
                        if (itemName != "Dummy Item"){
                            pcRecord.removeLine({sublistId: sublistId, line: line});
                            --line;
                            hasRemoveItem = true;
                        }
                    }
                    hasRemoveItem && pcRecord.save({ignoreMandatoryFields: true});
                }
            } catch (e) {
                log.debug("Exception on Map entrypoint while remove items for pc:" + pcId, e)
            }
        }
