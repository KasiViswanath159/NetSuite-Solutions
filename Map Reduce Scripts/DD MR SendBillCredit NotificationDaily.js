...
define(['N/record', 'N/runtime', 'N/search', 'N/file', 'N/format','N/email', 'N/log', 'N/render'],
	...
	(record, runtime, search, file, format,email, log, render) => {
			...
			try {
				...
				var vendorcreditSearchObj = search.create({
					...
					filters: [
						...
						"AND", 
						["vendor.custentity_dd_shortpay_daily_email","is","T"]
					],
					columns: [
						search.createColumn({
							name: "internalid",
				...
				log.debug('vendorArray map ', vendorArray);
					...
					try {
						...
						var mySearch = search.load({
							id: 'customsearch_dd_billcredit_added_sp_dly'
						});
						...
						var vendorName = search.lookupFields({
							...
							id: vendorArray,
							columns: ['companyname','custentity_dd_accuratepay_emails','internalid']
						});
                      ...
                      // Add a condition to exclude certain vendors from attaching CSV files
                      log.debug('144 line',+excludeVendor(vendorArray[0]));
                      if (!excludeVendor(vendorArray[0])) {
                        log.debug('excludeVendor(vendorArray[0])'+excludeVendor(vendorArray[0]));
        ...
        function excludeVendor(vendorId) {
            
             if (vendorId == '120099' || vendorId == '391690' || vendorId == '391689' || vendorId == '87549'){
                return true; 
             }
            return false; 
        }
